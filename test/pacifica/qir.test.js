...
test('opaque pagination, id upserts, shared cache and transcript projection',async()=>{const {service,calls}=provider();const [a,b]=await Promise.all([service.catalog(),service.catalog()]);assert.deepEqual(a,b);assert.equal(a.episodes.length,1);assert.equal(a.episodes[0].headline,'Corrected');assert.ok(calls.some(c=>c.url.includes('cursor=opaque%20cursor')));assert.equal(calls.length,3);assert.equal(calls[0].headers.Authorization,'Bearer secret-test');assert.doesNotMatch(JSON.stringify(a),/secret-test/);assert.equal((await service.transcript(id)).transcript,'A transcript');assert.equal((await service.transcript(id)).language,'Spanish','language reaches the browser');await service.transcript(id);assert.equal(calls.length,4);await assert.rejects(service.transcript('../bad'),/invalid_episode_id/);});
