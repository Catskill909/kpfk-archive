...
test('real HTTP archive serves every episode of scheduled programs only, exact show metadata, branding and durable restart', async t => {
  ...
  async function boot() {
    child = spawn(process.execPath, ['--require', preload, 'server.js'], { cwd: root,
      env: { ...cleanEnv(), STATION_PROFILE: profileFile, PACIFICA_TEST_LOCAL: '1', STUDIO_PASSWORD: 'test-studio', QIR_API_KEY: '', ARTWORK_WARM: 'off', AUDIO_CHECK: 'off', PORT: String(port), DATA_DIR: path.join(dir, 'data') },
