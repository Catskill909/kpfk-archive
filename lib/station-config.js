...
function validateProfile(input, { allowLocal = false } = {}) {
  ...
  const plugins = { discovery: false };
  if (input.plugins !== undefined) {
    required(object(input.plugins), 'plugins must be an object');
    if (input.plugins.discovery !== undefined) {
      required(typeof input.plugins.discovery === 'boolean', 'plugins.discovery must be boolean');
      plugins.discovery = input.plugins.discovery;
    }
  }
  ...
  return {
    ...
    primaryChannel: input.primaryChannel, feeds, liveStream, origins, links, social, assets, categories, plugins,
  };
}
