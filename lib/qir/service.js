...
function episode(raw, audioOrigins) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(raw.air_date || '') || !/^(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d$/.test(raw.air_start || '')) throw problem('invalid_episode_time');
  ...
  return { public_id: raw.public_id, show_key: raw.show_key, show_name: text(raw.show_name,1000),
    ...
    air_date: raw.air_date, air_start: raw.air_start, air_end: text(raw.air_end,30),
...
function createQirService({enabled = false, key = '', audioOrigins = ['https://archive.kpfk.org'],
  ...
  function status() { return { provider:'qir', station:'kpfk', configured:enabled && !!key,
    ...
    lastSuccess: catalog ? catalog.fetchedAt : null, chaptersSupported:false }; }
  ...
  async function getCatalog() {
    ...
    pending=(async()=>{
      try {
        ...
        const found=new Map(), cursors=new Set();let cursor='', complete=false, totalBytes=0;
        for(let page=0;page<25;page++) {
          ...
          for(const raw of payload.data){const e=episode(raw,audioOrigins);found.set(e.public_id,e);}
        ...
        lastError='provider_temporarily_unavailable';cooldown=Math.max(cooldown,now()+30000);
        if(catalog) return {...catalog,stale:true};
        throw problem(e.message==='catalog_exceeds_beta_limit'?e.message:lastError,503);
