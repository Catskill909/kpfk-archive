...
const ENTITIES = {
  ...
  shy: '', copy: '©', reg: '®', bull: '•', eacute: 'é', Eacute: 'É', aacute: 'á',
  iacute: 'í', oacute: 'ó', uacute: 'ú', ntilde: 'ñ', Ntilde: 'Ñ', uuml: 'ü',
  iquest: '¿', iexcl: '¡', laquo: '«', raquo: '»',
};
function decode(s) {
  return s.replace(/&(#x[0-9a-f]+|#\d+|[a-zA-Z]+);/gi, (whole, code) => {
    if (code[0] !== '#') return Object.hasOwn(ENTITIES, code) ? ENTITIES[code] : whole;
...
function plain(value) {
  ...
  for (let i = 0; i < 3; i++) { const next = decode(s); if (next === s) break; s = next; }
  return s.replace(/<(script|style)\b[^>]*>[\s\S]*?<\/\1\s*>/gi, '')
