...
const { createProbeStore } = require('./audio-probe');

function createService({ profile, dataDir, writeJsonAtomic, fetchImpl = fetch, now = Date.now, audioCheck = true }) {
  const states = new Map(), images = new Map();
  const probes = createProbeStore({ dataDir, fetchImpl, origins: profile.origins.audio, writeJsonAtomic });
  let activeFetches = 0;
  const waiting = [];
  ...
  async function get(s, force = false) {
    ...
    s.inFlight = (async () => {
      try {
        ...
        if (value.skipped && value.skipped.count && (!s.value || s.value.revision !== value.revision)) {
          const first = value.skipped.records.slice(0, 3).map(r => `${r.path}: ${r.issue}`).join('; ');
          console.warn(`[pacifica] ${s.name}: skipped ${value.skipped.count} malformed record(s) (+${value.skipped.droppedWithShow} with their show): ${first}`);
        }
  ...
  // Display policy (Paul, 2026-09-15; uploads added 2026-09-25): on-air programs
  // only when they are in KPFK's published schedule, plus every upload list (the
  // `2kpfk` group: podcasts and online editions such as Bike Talk Podcast and
  // BradCast w/ Brad Friedman, which are never in the schedule and were hidden
  // until then). On-air programs no longer scheduled stay in the catalog snapshot,
  // untouched, and are not served. `catalog()` remains a faithful mirror of the
  // feed; `archive()` is the view every listener-facing consumer reads.
  function primaryChannelKeys(data) {
    return new Set(Object.keys(data.directory).filter(k => k.split('.')[1] === profile.primaryChannel));
  }
  // Upload shows with at least one episode; the feed also lists empty upload records.
  function withUploads(data, onAirKeys) {
    const keys = new Set(onAirKeys);
    for (const r of data.shows) if (r.sho.split('.')[1] !== profile.primaryChannel) keys.add(r.sho);
    return keys;
  }
  // Episodes dated in the future (pre-uploaded, e.g. Politics Or Pedagogy the day before it
  // airs) are held back until their air time (Paul, 2026-09-26). `catalog()` still mirrors them.
  function scheduledView(data, onAirKeys, basis) {
    const keys = withUploads(data, onAirKeys), at = now() / 1000;
    const listed = data.shows.filter(r => keys.has(r.sho));
    const held = listed.filter(r => r.dt > at), failed = [], corrected = [];
    // What the audio file itself says (lib/pacifica/audio-probe.js; unknown until probed).
    // Failed recording: under 1 MB AND under 60 s of audio (or none), or missing — both size
    // and length, so a misread header can never hide a real episode. Hidden and recorded.
    // A duration off by more than a minute or 10% is replaced by the file's own, recorded.
    const shows = listed.filter(r => r.dt <= at).flatMap(r => {
      const p = probes.get(r.mp3);
      if (!p) return [r];
      if (p.missing || (p.bytes < 1e6 && (p.seconds === null || p.seconds < 60))) {
        failed.push({ id: r.id, show: r.upstreamAltId, dt: r.dt, bytes: p.bytes ?? null, seconds: p.seconds ?? null, missing: p.missing || null });
        return [];
      }
      if (p.seconds && Math.abs(p.seconds - r.durationSec) > Math.max(60, p.seconds * 0.1)) {
        corrected.push({ id: r.id, show: r.upstreamAltId, dt: r.dt, feed: r.durationSec, file: p.seconds });
        return [{ ...r, durationSec: p.seconds, length: hms(p.seconds) }];
      }
      return [r];
    });
    ...
    // Released held items change the listing too, so they are part of the revision.
    const revision = crypto.createHash('sha256').update(`${data.revision}\n${basis}\n${[...keys].sort().join('\n')}\n${held.map(r => r.id).sort().join('\n')}\n${JSON.stringify([failed, corrected])}`).digest('hex');
    return { ...data, shows, directory, count: shows.length, latest: shows.reduce((max, r) => Math.max(max, r.dt), 0),
      revision, filter: { basis, catalogEpisodes: data.count, hiddenEpisodes: data.count - listed.length,
        heldUntilAir: held.map(r => ({ id: r.id, show: r.upstreamAltId, airs: r.dt })),
        failedRecordings: failed, durationCorrected: corrected, audioChecked: probes.size() } };
  }
  ...
  async function archive(force = false) {
    ...
    try { keys = await scheduledKeys(force); }
      ...
      // independent services), so fall back to all of the on-air channel's shows
      // (uploads are served as always) and say so in `filter.basis`, /healthz and the log.
    ...
    // Check new audio files in the background; results apply from the next view.
    if (audioCheck) probes.run(data.shows.map(r => r.mp3)).catch(e => console.warn(`[pacifica] audio check stopped: ${e.message}`));
  ...
  function peekArchive() {
    ...
    // on-air channel (plus uploads) but label it "pending", not
  ...
  return {
    ...
    health() {
      return Object.fromEntries([...states].map(([key, s]) => [key, { ready: !!s.value,
        ...
        // Catalog only: malformed records left out of an accepted catalog (see normalizeCatalog).
        skipped: s.value && s.value.skipped ? { count: s.value.skipped.count, droppedWithShow: s.value.skipped.droppedWithShow,
          records: s.value.skipped.records.slice(0, 10) } : undefined }]));
    },
  };
}
