...
/**
 * An image shared by more than this many shows is not any show's own artwork.
 *
 * Found 2026-09-16: shows with no image had an EMPTY `photoUrl` in the feed on
 * 2026-09-14 (the pinned fixtures, 81 of them), and by 2026-09-16 Pacifica was
 * filling that gap with a generic station picture — `…/pix/KPFK_med.jpg` on 78
 * shows. Treating any URL as artwork turned "20 scheduled programs have no
 * artwork" into "0". No real image in the catalog is shared by even two shows,
 * so a count works for every station without knowing each one's file name.
 */
...
const NOTES = {
  shows: {
    ...
    has_artwork: 'true when the catalog gives the show its OWN image. An image shared by 4 or more shows (the feed\'s generic station picture) does not count; the JSON summary lists any such image.',
...
function buildCoverage(o) {
  ...
  for (const info of Object.values(o.directory)) {
    if (info && info.photoUrl) urlUse.set(info.photoUrl, (urlUse.get(info.photoUrl) || 0) + 1);
  }
  ...
  const shows = Object.keys(o.directory).sort().map((key) => {
    ...
    return {
      ...
      has_artwork: !!info.photoUrl && !generic.has(info.photoUrl),
