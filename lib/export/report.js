...
<section class="section" id="archive">
  ...
  <p class="note">Episodes listeners can play today whose air date in ${esc(I.manifest.station_timezone)} falls in the period. Older episodes the station no longer holds are not counted.</p>
</section>`;
}
...
function coverageSection(C) {
  const known = C.manifest.schedule_known;
  const scheduled = known ? C.shows.filter((s) => s.in_published_schedule === true) : C.shows.filter((s) => s.shown_to_listeners);
  const noArt = scheduled.filter((s) => !s.has_artwork);
  const noDesc = scheduled.filter((s) => !s.has_description);
  const noEps = scheduled.filter((s) => s.episodes_in_catalog === 0);
  const stale = scheduled.filter((s) => s.episodes_in_catalog > 0 && s.days_since_newest_episode > 30);
  return `
<section class="section" id="coverage">
  <h2>Program data gaps</h2>
  <p class="note">As of ${esc(C.manifest.as_of_utc.slice(0, 10))}, for the ${num(scheduled.length)} programs ${known ? 'in the published schedule' : 'shown to listeners (the schedule could not be read)'}. These come from Pacifica&rsquo;s feed; they are fixed in Confessor, not in this app.</p>
  <div class="tiles">
    ...
    <div class="tile"><div class="tile-value">${num(noEps.length)}</div><div class="tile-label">No episodes</div></div>
    <div class="tile"><div class="tile-value">${num(stale.length)}</div><div class="tile-label">None in 30 days</div></div>
  </div>
  <h3>Scheduled without artwork</h3>${listOf(noArt)}
  <h3>Scheduled with no episodes in the archive</h3>${listOf(noEps)}
