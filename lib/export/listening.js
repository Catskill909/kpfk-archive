 ...
 *
 * Periods are calendar months or all time, never rolling windows (a board asks
 * for "September"). Days are the UTC days the counters were bucketed into at
...
const COLUMN_NOTES = {
  ...
  shows: {
    ...
    show_title: 'The show\'s title from the feed. Empty when no feed names the show — never filled with the id.',
...
function monthStartMs(m) { return Date.UTC(+m.slice(0, 4), +m.slice(5, 7) - 1, 1); }

/** Every UTC date in the period, oldest first, never past `today`. */
function periodDates(period, months, today) {
  const first = period === 'all' ? months[0] : period;
  const endMs = period === 'all'
    ? Date.parse(today + 'T00:00:00Z')
    : Math.min(Date.UTC(+period.slice(0, 4), +period.slice(5, 7), 0), Date.parse(today + 'T00:00:00Z'));
  const out = [];
  for (let t = monthStartMs(first); t <= endMs; t += 86400000) out.push(isoDay(t));
  return out;
}
 ...
 * @param {string} o.period           'YYYY-MM' (validated by the caller) or 'all'
 * @param {string[]} o.months         stats months on disk, ascending
 * @param {(m: string) => object} o.monthDays  a month's day records
 * @param {string} o.today            today's UTC date, YYYY-MM-DD
...
function buildListeningExport(o) {
  const dates = periodDates(o.period, o.months, o.today);
  const cache = new Map();
  const recFor = (d) => {
    const m = d.slice(0, 7);
  ...
  return {
    manifest: {
      ...
      period: o.period,
      first_date_utc: dates[0],
      last_date_utc: dates[dates.length - 1],
...
function manifestText(m) {
  const lines = [
    ...
    '',
    `Station:          ${m.station}`,
    `Period:           ${m.period} (${m.first_date_utc} to ${m.last_date_utc}, ${m.days_covered} days)`,
  ...
  for (const [table, cols] of Object.entries(m.tables)) {
    lines.push('', `Table "${table}" (${m.station}-listening-${table}-${m.period}.csv)`);
    for (const c of cols) lines.push(`  ${c.column}: ${c.meaning}`);
  }
  return lines.join('\r\n') + '\r\n';
}

module.exports = { buildListeningExport, manifestText, dayCounters, periodDates, COLUMNS, SCHEMA_VERSION, NO_IDENTIFIER };
